...
public class RenderBlobfish
extends MobRenderer<EntityBlobfish, EntityModel<EntityBlobfish>> {
    ...
    public RenderBlobfish(EntityRendererProvider.Context renderManagerIn) {
        super(renderManagerIn, (EntityModel)new ModelBlobfish(), 0.35f);
    }

    protected void scale(EntityBlobfish entitylivingbaseIn, PoseStack matrixStackIn, float partialTickTime) {
        this.f_115290_ = entitylivingbaseIn.isDepressurized() ? this.modelDepressurized : this.modelFish;
        matrixStackIn.m_85841_(entitylivingbaseIn.getBlobfishScale(), entitylivingbaseIn.getBlobfishScale(), entitylivingbaseIn.getBlobfishScale());
    }

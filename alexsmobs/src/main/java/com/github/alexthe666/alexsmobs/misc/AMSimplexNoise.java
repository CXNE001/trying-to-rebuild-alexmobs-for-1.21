...
public class AMSimplexNoise {
    ...
    public static double noise(double xin, double yin) {
        ...
        double t;
        double s = (xin + yin) * F2;
        int i = AMSimplexNoise.fastfloor(xin + s);
        double X0 = (double)i - (t = (double)(i + (j = AMSimplexNoise.fastfloor(yin + s))) * G2);
        double x0 = xin - X0;
        ...
        if (t0 < 0.0) {
            ...
            t0 *= t0;
            n0 = t0 * t0 * AMSimplexNoise.dot(grad3[gi0], x0, y0);
        }
        ...
        if (t1 < 0.0) {
            ...
            t1 *= t1;
            n1 = t1 * t1 * AMSimplexNoise.dot(grad3[gi1], x1, y1);
        }
        ...
        if (t2 < 0.0) {
            ...
            t2 *= t2;
            n2 = t2 * t2 * AMSimplexNoise.dot(grad3[gi2], x2, y2);
        }
    ...
    public static double noise(double xin, double yin, double zin) {
        ...
        int i = AMSimplexNoise.fastfloor(xin + s);
        int j = AMSimplexNoise.fastfloor(yin + s);
        int k = AMSimplexNoise.fastfloor(zin + s);
        ...
        if (t0 < 0.0) {
            ...
            t0 *= t0;
            n0 = t0 * t0 * AMSimplexNoise.dot(grad3[gi0], x0, y0, z0);
        }
        ...
        if (t1 < 0.0) {
            ...
            t1 *= t1;
            n1 = t1 * t1 * AMSimplexNoise.dot(grad3[gi1], x1, y1, z1);
        }
        ...
        if (t2 < 0.0) {
            ...
            t2 *= t2;
            n2 = t2 * t2 * AMSimplexNoise.dot(grad3[gi2], x2, y2, z2);
        }
        ...
        if (t3 < 0.0) {
            ...
            t3 *= t3;
            n3 = t3 * t3 * AMSimplexNoise.dot(grad3[gi3], x3, y3, z3);
        }
    ...
    public static double noise(double x, double y, double z, double w) {
        ...
        double n0;
        double s = (x + y + z + w) * F4;
        int i = AMSimplexNoise.fastfloor(x + s);
        int j = AMSimplexNoise.fastfloor(y + s);
        int k = AMSimplexNoise.fastfloor(z + s);
        int l = AMSimplexNoise.fastfloor(w + s);
        ...
        if (t0 < 0.0) {
            ...
            t0 *= t0;
            n0 = t0 * t0 * AMSimplexNoise.dot(grad4[gi0], x0, y0, z0, w0);
        }
        ...
        if (t1 < 0.0) {
            ...
            t1 *= t1;
            n1 = t1 * t1 * AMSimplexNoise.dot(grad4[gi1], x1, y1, z1, w1);
        }
        ...
        if (t2 < 0.0) {
            ...
            t2 *= t2;
            n2 = t2 * t2 * AMSimplexNoise.dot(grad4[gi2], x2, y2, z2, w2);
        }
        ...
        if (t3 < 0.0) {
            ...
            t3 *= t3;
            n3 = t3 * t3 * AMSimplexNoise.dot(grad4[gi3], x3, y3, z3, w3);
        }
        ...
        if (t4 < 0.0) {
            ...
            t4 *= t4;
            n4 = t4 * t4 * AMSimplexNoise.dot(grad4[gi4], x4, y4, z4, w4);
        }
    ...
    static {
        for (int i = 0; i < 512; ++i) {
            AMSimplexNoise.perm[i] = p[i & 0xFF];
            AMSimplexNoise.permMod12[i] = (short)(perm[i] % 12);
        }

...
public class CABAC {
    ...
    public int readCoeffs(MDecoder decoder, BlockType blockType, int[] out, int first, int num, int[] reorder, int[] scMapping, int[] lscMapping) {
        ...
        for (int numCoeff = 0; numCoeff < num - 1; ++numCoeff) {
            boolean bl = sigCoeff[numCoeff] = decoder.decodeBin(blockType.sigCoeffFlagCtxOff + scMapping[numCoeff]) == 1;
    ...
    private int readCoeffAbsLevel(MDecoder decoder, BlockType blockType, int numDecodAbsLevelGt1, int numDecodAbsLevelEq1) {
        ...
        if ((val += b) == 14) {
            ...
            do {
                ++log;
            } while ((b = decoder.decodeBinBypass()) != 0);
            while (log >= 0) {
    ...
    public void writeCoeffs(MEncoder encoder, BlockType blockType, int[] _out, int first, int num, int[] reorder) {
        ...
        for (int i2 = 0; i2 < num; ++i2) {
            this.tmp[i2] = _out[reorder[first + i2]];
        }
        int numCoeff = 0;
        for (i = 0; i < num; ++i) {
        ...
        for (i = 0; i < Math.min(numCoeff, num - 1); ++i) {
            if (this.tmp[i] != 0) {
    ...
    private void writeMBType16x16(MEncoder encoder, int mbType) {
        ...
        if (mbType < 4) {
            ...
            encoder.encodeBin(8, (mbType -= 4) >> 2);
    ...
    public int readMBQpDelta(MDecoder decoder, MBType prevMbType) {
        int ctx = 60;
        int n = prevMbType == null || prevMbType == MBType.I_PCM || prevMbType != MBType.I_16x16 && this.prevCBP == 0 || this.prevMbQpDelta == 0 ? 0 : 1;
        int val = 0;
        if (decoder.decodeBin(ctx += n) == 1) {
            ++val;
    ...
    public int readIntraChromaPredMode(MDecoder decoder, int mbX, MBType left, MBType top, boolean leftAvailable, boolean topAvailable) {
        ...
        int mode = decoder.decodeBin(ctx += !topAvailable || top == null || !top.isIntra() || this.chromaPredModeTop[mbX] == 0 ? 0 : 1) == 0 ? 0 : (decoder.decodeBin(67) == 0 ? 1 : (decoder.decodeBin(67) == 0 ? 2 : 3));
    ...
    public void writeIntraChromaPredMode(MEncoder encoder, int mbX, MBType left, MBType top, boolean leftAvailable, boolean topAvailable, int mode) {
        ...
        encoder.encodeBin(ctx += !topAvailable || !top.isIntra() || this.chromaPredModeTop[mbX] == 0 ? 0 : 1, mode-- == 0 ? 0 : 1);
    ...
    public int readCodedBlockFlagLumaDC(MDecoder decoder, int mbX, MBType left, MBType top, boolean leftAvailable, boolean topAvailable, MBType cur) {
        ...
        this.codedBlkDCLeft[0] = decoded = decoder.decodeBin(BlockType.LUMA_16_DC.codedBlockCtxOff + tLeft + 2 * tTop);
    ...
    public int readCodedBlockFlagChromaDC(MDecoder decoder, int mbX, int comp, MBType left, MBType top, boolean leftAvailable, boolean topAvailable, int leftCBPChroma, int topCBPChroma, MBType cur) {
        ...
        this.codedBlkDCLeft[comp] = decoded = decoder.decodeBin(BlockType.CHROMA_DC.codedBlockCtxOff + tLeft + 2 * tTop);
    ...
    public int readCodedBlockFlagLumaAC(MDecoder decoder, BlockType blkType, int blkX, int blkY, int comp, MBType left, MBType top, boolean leftAvailable, boolean topAvailable, int leftCBPLuma, int topCBPLuma, int curCBPLuma, MBType cur) {
        ...
        int tLeft = blkOffLeft == 0 ? this.condTerm(cur, leftAvailable, left, left != null && left != MBType.I_PCM && this.cbp(leftCBPLuma, 3, blkOffTop), this.codedBlkLeft[comp][blkOffTop]) : this.condTerm(cur, true, cur, this.cbp(curCBPLuma, blkOffLeft - 1, blkOffTop), this.codedBlkLeft[comp][blkOffTop]);
        int tTop = blkOffTop == 0 ? this.condTerm(cur, topAvailable, top, top != null && top != MBType.I_PCM && this.cbp(topCBPLuma, blkOffLeft, 3), this.codedBlkTop[comp][blkX]) : this.condTerm(cur, true, cur, this.cbp(curCBPLuma, blkOffLeft, blkOffTop - 1), this.codedBlkTop[comp][blkX]);
        this.codedBlkLeft[comp][blkOffTop] = decoded = decoder.decodeBin(blkType.codedBlockCtxOff + tLeft + 2 * tTop);
    ...
    public int readCodedBlockFlagLuma64(MDecoder decoder, int blkX, int blkY, int comp, MBType left, MBType top, boolean leftAvailable, boolean topAvailable, int leftCBPLuma, int topCBPLuma, int curCBPLuma, MBType cur, boolean is8x8Left, boolean is8x8Top) {
        ...
        int blkOffLeft = blkX & 3;
        int blkOffTop = blkY & 3;
        int tLeft = blkOffLeft == 0 ? this.condTerm(cur, leftAvailable, left, left != null && left != MBType.I_PCM && is8x8Left && this.cbp(leftCBPLuma, 3, blkOffTop), this.codedBlkLeft[comp][blkOffTop]) : this.condTerm(cur, true, cur, this.cbp(curCBPLuma, blkOffLeft - 1, blkOffTop), this.codedBlkLeft[comp][blkOffTop]);
        int tTop = blkOffTop == 0 ? this.condTerm(cur, topAvailable, top, top != null && top != MBType.I_PCM && is8x8Top && this.cbp(topCBPLuma, blkOffLeft, 3), this.codedBlkTop[comp][blkX]) : this.condTerm(cur, true, cur, this.cbp(curCBPLuma, blkOffLeft, blkOffTop - 1), this.codedBlkTop[comp][blkX]);
        this.codedBlkLeft[comp][blkOffTop] = decoded = decoder.decodeBin(BlockType.LUMA_64.codedBlockCtxOff + tLeft + 2 * tTop);
        this.codedBlkTop[comp][blkX] = decoded;
        return decoded;
    }
    ...
    public int readCodedBlockFlagChromaAC(MDecoder decoder, int blkX, int blkY, int comp, MBType left, MBType top, boolean leftAvailable, boolean topAvailable, int leftCBPChroma, int topCBPChroma, MBType cur) {
        ...
        int tLeft = blkOffLeft == 0 ? this.condTerm(cur, leftAvailable, left, left != null && left != MBType.I_PCM && (leftCBPChroma & 2) != 0, this.codedBlkLeft[comp][blkOffTop]) : this.condTerm(cur, true, cur, true, this.codedBlkLeft[comp][blkOffTop]);
        int tTop = blkOffTop == 0 ? this.condTerm(cur, topAvailable, top, top != null && top != MBType.I_PCM && (topCBPChroma & 2) != 0, this.codedBlkTop[comp][blkX]) : this.condTerm(cur, true, cur, true, this.codedBlkTop[comp][blkX]);
        this.codedBlkLeft[comp][blkOffTop] = decoded = decoder.decodeBin(BlockType.CHROMA_AC.codedBlockCtxOff + tLeft + 2 * tTop);
        this.codedBlkTop[comp][blkX] = decoded;
        return decoded;
    }
    ...
    public int readMVD(MDecoder decoder, int comp, boolean leftAvailable, boolean topAvailable, MBType leftType, MBType topType, H264Const.PartPred leftPred, H264Const.PartPred topPred, H264Const.PartPred curPred, int mbX, int partX, int partY, int partW, int partH, int list) {
        ...
        int absMvdComp = !leftAvailable || leftType == null || leftType.isIntra() || !predEqA ? 0 : Math.abs(this.mvdLeft[list][comp][partY]);
        int b = decoder.decodeBin(ctx + ((absMvdComp += !topAvailable || topType == null || topType.isIntra() || !predEqB ? 0 : Math.abs(this.mvdTop[list][comp][partAbsX])) < 3 ? 0 : (absMvdComp > 32 ? 2 : 1)));
    ...
    public int readRefIdx(MDecoder mDecoder, boolean leftAvailable, boolean topAvailable, MBType leftType, MBType topType, H264Const.PartPred leftPred, H264Const.PartPred topPred, H264Const.PartPred curPred, int mbX, int partX, int partY, int partW, int partH, int list) {
        ...
        int b0 = mDecoder.decodeBin(54 + ctA + 2 * (ctB = !topAvailable || topType == null || topType.isIntra() || !predEqB || this.refIdxTop[list][partAbsX] == 0 ? 0 : 1));
        if (b0 == 0) {
            val = 0;
        } else {
    ...
    public boolean readMBSkipFlag(MDecoder mDecoder, SliceType slType, boolean leftAvailable, boolean topAvailable, int mbX) {
        ...
        this.skipFlagLeft = this.skipFlagsTop[mbX] = (ret = mDecoder.decodeBin(base + (leftAvailable && !this.skipFlagLeft ? 1 : 0) + (topAvailable && !this.skipFlagsTop[mbX] ? 1 : 0)) == 1);
        return ret;
    }

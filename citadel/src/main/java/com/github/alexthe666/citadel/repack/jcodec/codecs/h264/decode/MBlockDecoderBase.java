...
import com.github.alexthe666.citadel.repack.jcodec.codecs.h264.decode.ChromaPredictionBuilder;
...
public class MBlockDecoderBase {
    ...
    protected static int[][] initScalingMatrix(SliceHeader sh2) {
        ...
        for (int i = 0; i < 8; ++i) {
            ...
            if (sh2.pps.extended == null || sh2.pps.extended.scalingMatrix == null || sh2.pps.extended.scalingMatrix[i] == null) continue;
    ...
    public void decodeChroma(MBlock mBlock, int mbX, int mbY, boolean leftAvailable, boolean topAvailable, Picture mb, int qp) {
        ...
        int qp1 = MBlockDecoderBase.calcQpChroma(qp, this.s.chromaQpOffset[0]);
        int qp2 = MBlockDecoderBase.calcQpChroma(qp, this.s.chromaQpOffset[1]);
        ...
        for (int blk8x8 = 0; blk8x8 < 4; ++blk8x8) {
            for (int list = 0; list < 2; ++list) {
                ...
                for (int blk4x4 = 0; blk4x4 < 4; ++blk4x4) {
                    ...
                    Frame ref = refs[list][H264Utils.Mv.mvRef(mv)];
                    int blkPox = (i & 3) << 1;
                    int blkPoy = i >> 2 << 1;

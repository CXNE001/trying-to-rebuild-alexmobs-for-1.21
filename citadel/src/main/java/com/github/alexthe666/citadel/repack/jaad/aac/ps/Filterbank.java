...
implements PSTables {
    ...
    Filterbank(int numTimeSlotsRate) {
        ...
        this.buffer = new float[5][this.frame_len][2];
    ...
    void hybrid_analysis(float[][][] X, float[][][] X_hybrid, boolean use34, int numTimeSlotsRate) {
        ...
        for (int band = 0; band < qmf_bands; ++band) {
            ...
            switch (resolution[band]) {
                case 2: {
                    Filterbank.channel_filter2(this.frame_len, p2_13_20, this.work, this.temp);
                    break;
                }
                case 4: {
                    Filterbank.channel_filter4(this.frame_len, p4_13_34, this.work, this.temp);
                    break;
                }
                case 8: {
    ...
    void channel_filter8(int frame_len, float[] filter, float[][] buffer, float[][][] X_hybrid) {
        ...
        for (int i = 0; i < frame_len; ++i) {
            ...
            Filterbank.DCT3_4_unscaled(x, x);
            X_hybrid[i][7][0] = x[0];
            X_hybrid[i][5][0] = x[2];
            X_hybrid[i][3][0] = x[3];
            X_hybrid[i][1][0] = x[1];

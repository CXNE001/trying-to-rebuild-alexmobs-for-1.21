...
import com.github.alexthe666.citadel.repack.jcodec.movtool.Strip;
...
public class Paste {
    ...
    public static void main1(String[] args) throws Exception {
        ...
        try {
            ...
            MP4Util.Movie toMov = MP4Util.createRefFullMovie((SeekableByteChannel)to, "file://" + toFile.getCanonicalPath());
            MP4Util.Movie fromMov = MP4Util.createRefFullMovie((SeekableByteChannel)from, "file://" + fromFile.getCanonicalPath());
            ...
            MP4Util.writeFullMovie((SeekableByteChannel)out, toMov);
        }
        finally {
            if (to != null) {
                to.close();
    ...
    private int[][] findMatches(TrakBox[] fromTracks, TrakBox[] toTracks) {
        ...
        Arrays.fill(f2t, -1);
        Arrays.fill(t2f, -1);
        block0: for (int i = 0; i < fromTracks.length; ++i) {
            if (f2t[i] != -1) continue;
            for (int j = 0; j < toTracks.length; ++j) {
                if (t2f[j] != -1 || !this.matches(fromTracks[i], toTracks[j])) continue;
                f2t[i] = j;
                t2f[j] = i;
                continue block0;
            }

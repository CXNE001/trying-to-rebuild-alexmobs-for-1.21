...
extends VideoEncoder {
    ...
    public static ProresEncoder createProresEncoder(String profile, boolean interlaced) {
        return new ProresEncoder(profile == null ? Profile.HQ : Profile.valueOf(profile), interlaced);
    }
    ...
    static final void writeDCCoeffs(BitWriter bits, int[] qMat, int[] _in, int blocksPerSlice) {
        int prevDc = ProresEncoder.qScale(qMat, 0, _in[0] - 16384);
        ...
        while (i < blocksPerSlice) {
            int newDc = ProresEncoder.qScale(qMat, 0, _in[idx] - 16384);
            int delta = newDc - prevDc;
            int newCode = ProresEncoder.toGolumbSign(ProresEncoder.getLevel(delta), ProresEncoder.diffSign(delta, sign));
    ...
    static final void writeACCoeffs(BitWriter bits, int[] qMat, int[] _in, int blocksPerSlice, int[] scan, int maxCoeff) {
        ...
        for (int i = 1; i < maxCoeff; ++i) {
            ...
            for (int j = 0; j < blocksPerSlice; ++j) {
                int val = ProresEncoder.qScale(qMat, indp, _in[(j << 6) + indp]);
                if (val == 0) {
                    ++run;
                    continue;
                }
    ...
    public static final class Profile {
        ...
        private static final Profile[] _values = new Profile[]{PROXY, LT, STANDARD, HQ};
        final String name;
        final int[] qmatLuma;

...
extends Block {
    ...
    protected int parseICSInfo(BitReader _in) {
        ...
        if (this.windowSequence == WindowSequence.EIGHT_SHORT_SEQUENCE.ordinal()) {
            ...
            for (int i = 0; i < 7; ++i) {
                if (_in.read1Bit() != 0) {
                    int n = this.num_window_groups - 1;
                    this.group_len[n] = this.group_len[n] + 1;
                    continue;
                }
        ...
        for (int g = 0; g < this.num_window_groups; ++g) {
            ...
            while (i < this.maxSfb) {
                ...
                if (this.band_type[idx] == BandType.NOISE_BT.ordinal()) {
                    while (i < run_end) {
                        offset[1] = noise_flag-- > 0 ? offset[1] + (_in.readNBit(9) - 256) : offset[1] + (vlc.readVLC(_in) - 60);
    ...
    private Tns decodeTns(BitReader _in) {
        int is8;
        int n = is8 = this.windowSequence == WindowSequence.EIGHT_SHORT_SEQUENCE.ordinal() ? 1 : 0;
    ...
    private void readBandType3And4(BitReader _in, float[] coef, int idx, int g, int sfb, float[] vq, VLC vlc) {
        ...
        while (group < g_len) {
            ...
            do {
                int cb_idx;
                int nnz;
                int bits = (nnz = (cb_idx = vlc.readVLC(_in)) >> 8 & 0xF) == 0 ? 0 : _in.readNBit(nnz);
    ...
    private void readBandType7Through10(BitReader _in, float[] coef, int idx, int g, int sfb, float[] vq, VLC vlc) {
        ...
        while (group < g_len) {
            ...
            do {
                int cb_idx;
                int nnz;
                int bits = (nnz = (cb_idx = vlc.readVLC(_in)) >> 8 & 0xF) == 0 ? 0 : _in.readNBit(nnz) << (cb_idx >> 12);
    ...
    private void readOther(BitReader _in, float[] coef, int idx, int g, int sfb, float[] vq, VLC vlc) {
        ...
        while (group < g_len) {
            ...
            do {
                int cb_idx;
                if ((cb_idx = vlc.readVLC(_in)) == 0) continue;
                int nnz = cb_idx >> 12;
                int nzt = cb_idx >> 8;
                ...
                for (int j = 0; j < 2; ++j) {
                    if ((nzt & 1 << j) != 0) {
                        ...
                        _in.skip(b + 1);
                        int n = (1 << (b += 4)) + _in.readNBit(b);
                        coef[cf++] = MathUtil.cubeRoot(n) | bits & Integer.MIN_VALUE;
                        bits <<= 1;
                    } else {

...
public class SliceHeaderReader {
    ...
    private static void readDecoderPicMarking(NALUnit nalUnit, SliceHeader sh, BitReader _in) {
        if (nalUnit.type == NALUnitType.IDR_SLICE) {
            ...
            if (adaptiveRefPicMarkingModeFlag) {
                ...
                sh.refPicMarkingNonIDR = new RefPicMarking(mmops.toArray(new RefPicMarking.Instruction[0]));
            }
        }
    }
    ...
    private static void readWeightOffset(SeqParameterSet sps, PictureParameterSet pps, SliceHeader sh, BitReader _in, int[] numRefs, int list) {
        for (int i = 0; i < numRefs[list]; ++i) {
            ...
            if (sps.chromaFormatIdc == ColorSpace.MONO || !(chromaWeightL0Flag = CAVLCReader.readBool(_in, "SH: chroma_weight_l0_flag"))) continue;
    ...
    private static void readRefPicListReordering(SliceHeader sh, BitReader _in) {
        boolean refPicListReorderingFlagL1;
        boolean refPicListReorderingFlagL0;
        sh.refPicReordering = new int[2][][];
        if (sh.sliceType.isInter() && (refPicListReorderingFlagL0 = CAVLCReader.readBool(_in, "SH: ref_pic_list_reordering_flag_l0"))) {
            sh.refPicReordering[0] = SliceHeaderReader.readReorderingEntries(_in);
        }
        if (sh.sliceType == SliceType.B && (refPicListReorderingFlagL1 = CAVLCReader.readBool(_in, "SH: ref_pic_list_reordering_flag_l1"))) {
    ...
    private static int[][] readReorderingEntries(BitReader _in) {
        ...
        while ((idc = CAVLCReader.readUEtrace(_in, "SH: reordering_of_pic_nums_idc")) != 3) {
            ops.add(idc);

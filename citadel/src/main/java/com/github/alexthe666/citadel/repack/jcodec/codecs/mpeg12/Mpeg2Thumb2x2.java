...
import com.github.alexthe666.citadel.repack.jcodec.codecs.mpeg12.MPEGPredQuad;
...
extends MPEGDecoder {
    ...
    protected void blockIntra(BitReader bits, VLC vlcCoeff, int[] block, int[] intra_dc_predictor, int blkIdx, int[] scan, int escSize, int intra_dc_mult, int qScale, int[] qmat) {
        ...
        int delta = size != 0 ? Mpeg2Thumb2x2.mpegSigned(bits, size) : 0;
        ...
        while (idx < 6 && (readVLC = vlcCoeff.readVLC(bits)) != 2048) {
            int level;
            level = readVLC == 2049 ? ((level = Mpeg2Thumb2x2.twosSigned(bits, escSize) * qScale * qmat[idx += bits.readNBit(6) + 1]) >= 0 ? level >> 4 : -(-level >> 4)) : Mpeg2Thumb2x2.toSigned((readVLC & 0x3F) * qScale * qmat[idx += (readVLC >> 6) + 1] >> 4, bits.read1Bit());
    ...
    protected void blockInter(BitReader bits, VLC vlcCoeff, int[] block, int[] scan, int escSize, int qScale, int[] qmat) {
        ...
        if (vlcCoeff == MPEGConst.vlcCoeff0 && bits.checkNBit(1) == 1) {
            bits.read1Bit();
            block[0] = Mpeg2Thumb2x2.toSigned(Mpeg2Thumb2x2.quantInter(1, qScale * qmat[0]), bits.read1Bit());
            ++idx;
        } else {
            block[0] = 0;
        }
        int readVLC = 0;
        while (idx < 6 && (readVLC = vlcCoeff.readVLC(bits)) != 2048) {
            int ac = readVLC == 2049 ? Mpeg2Thumb2x2.quantInterSigned(Mpeg2Thumb2x2.twosSigned(bits, escSize), qScale * qmat[idx += bits.readNBit(6) + 1]) : Mpeg2Thumb2x2.toSigned(Mpeg2Thumb2x2.quantInter(readVLC & 0x3F, qScale * qmat[idx += (readVLC >> 6) + 1]), bits.read1Bit());
            block[scan[idx]] = ac;
        }
        if (readVLC != 2048) {
            this.finishOff(bits, idx, vlcCoeff, escSize);
    ...
    protected void mapBlock(int[] block, int[] out, int blkIdx, int dctType, int chromaFormat) {
        ...
        out[n] = out[n] + block[0];
        int n2 = off + 1;
        out[n2] = out[n2] + block[1];
        int n3 = off + stride;
        out[n3] = out[n3] + block[2];
        int n4 = off + stride + 1;
        out[n4] = out[n4] + block[3];
    }
    ...
    protected void putSub(byte[] big, int off, int stride, int[] block, int mbW, int mbH) {
        int blOff = 0;
        if (mbW == 1) {
            big[off] = Mpeg2Thumb2x2.clipTo8Bit(block[blOff]);
            big[off + 1] = Mpeg2Thumb2x2.clipTo8Bit(block[blOff + 1]);
            big[off + stride] = Mpeg2Thumb2x2.clipTo8Bit(block[blOff + 2]);
            big[off + stride + 1] = Mpeg2Thumb2x2.clipTo8Bit(block[blOff + 3]);
            if (mbH == 2) {
                big[off += stride << 1] = Mpeg2Thumb2x2.clipTo8Bit(block[blOff + 4]);
                big[off + 1] = Mpeg2Thumb2x2.clipTo8Bit(block[blOff + 5]);
                big[off + stride] = Mpeg2Thumb2x2.clipTo8Bit(block[blOff + 6]);
                big[off + stride + 1] = Mpeg2Thumb2x2.clipTo8Bit(block[blOff + 7]);
            }
        } else {
            for (int i = 0; i < 4; ++i) {
                big[off] = Mpeg2Thumb2x2.clipTo8Bit(block[blOff]);
                big[off + 1] = Mpeg2Thumb2x2.clipTo8Bit(block[blOff + 1]);
                big[off + 2] = Mpeg2Thumb2x2.clipTo8Bit(block[blOff + 2]);
                big[off + 3] = Mpeg2Thumb2x2.clipTo8Bit(block[blOff + 3]);
                off += stride;
                blOff += 4;
            }
        }

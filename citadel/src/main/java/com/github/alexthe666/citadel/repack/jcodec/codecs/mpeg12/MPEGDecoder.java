...
extends VideoDecoder {
    ...
    public int decodeMacroblock(PictureHeader ph, Context context, int prevAddr, int[] qScaleCode, byte[][] buf, int stride, BitReader bits, int vertOff, int vertStep, MPEGPred pred) {
        ...
        int n = cbp = mbType.macroblock_intra == 1 ? 4095 : 0;
    ...
    protected void mapBlock(int[] block, int[] out, int blkIdx, int dctType, int chromaFormat) {
        ...
        while (i < 8) {
            int n = off;
            out[n] = out[n] + block[coeff];
            int n2 = off + 1;
            out[n2] = out[n2] + block[coeff + 1];
            int n3 = off + 2;
            out[n3] = out[n3] + block[coeff + 2];
            int n4 = off + 3;
            out[n4] = out[n4] + block[coeff + 3];
            int n5 = off + 4;
            out[n5] = out[n5] + block[coeff + 4];
            int n6 = off + 5;
            out[n6] = out[n6] + block[coeff + 5];
            int n7 = off + 6;
            out[n7] = out[n7] + block[coeff + 6];
            int n8 = off + 7;
            out[n8] = out[n8] + block[coeff + 7];
            off += stride;
            ++i;
            coeff += 8;
        }
    ...
    protected void blockIntra(BitReader bits, VLC vlcCoeff, int[] block, int[] intra_dc_predictor, int blkIdx, int[] scan, int escSize, int intra_dc_mult, int qScale, int[] qmat) {
        ...
        while (idx < 64 && (readVLC = vlcCoeff.readVLC(bits)) != 2048) {
            int level;
            level = readVLC == 2049 ? ((level = MPEGDecoder.twosSigned(bits, escSize) * qScale * qmat[idx += bits.readNBit(6) + 1]) >= 0 ? level >> 4 : -(-level >> 4)) : MPEGDecoder.toSigned((readVLC & 0x3F) * qScale * qmat[idx += (readVLC >> 6) + 1] >> 4, bits.read1Bit());
    ...
    protected void blockInter(BitReader bits, VLC vlcCoeff, int[] block, int[] scan, int escSize, int qScale, int[] qmat) {
        ...
        while (idx < 64 && (readVLC = vlcCoeff.readVLC(bits)) != 2048) {
            int ac = readVLC == 2049 ? MPEGDecoder.quantInterSigned(MPEGDecoder.twosSigned(bits, escSize), qScale * qmat[idx += bits.readNBit(6) + 1]) : MPEGDecoder.toSigned(MPEGDecoder.quantInter(readVLC & 0x3F, qScale * qmat[idx += (readVLC >> 6) + 1]), bits.read1Bit());
    ...
    public static int getSequenceNumber(ByteBuffer data) {
        PictureHeader ph = MPEGDecoder.getPictureHeader(data);
        if (ph == null) {
            return -1;
        }
    ...
    public static PictureHeader getPictureHeader(ByteBuffer data) {
        ByteBuffer bb = MPEGDecoder.getRawPictureHeader(data);
        if (bb == null) {
            return null;
        }
    ...
    public VideoCodecMeta getCodecMeta(ByteBuffer data) {
        ByteBuffer codecPrivate = MPEGDecoder.getSequenceHeader(data.duplicate());

...
class HFGeneration {
    ...
    private static void calc_chirp_factors(SBR sbr, int ch) {
        for (int i = 0; i < sbr.N_Q; ++i) {
            ...
            sbr.bwArray[ch][i] = sbr.bwArray[ch][i] < sbr.bwArray_prev[ch][i] ? sbr.bwArray[ch][i] * 0.75f + sbr.bwArray_prev[ch][i] * 0.25f : sbr.bwArray[ch][i] * 0.90625f + sbr.bwArray_prev[ch][i] * 0.09375f;
    ...
    private static void patch_construction(SBR sbr) {
        ...
        do {
            int odd;
            int j = k + 1;
            while ((sb = sbr.f_master[--j]) > sbr.k0 - 1 + msb - (odd = (sb - 2 + sbr.k0) % 2)) {
            }
            ...
            if (sbr.f_master[k] - sb >= 3) continue;
            k = sbr.N_master;

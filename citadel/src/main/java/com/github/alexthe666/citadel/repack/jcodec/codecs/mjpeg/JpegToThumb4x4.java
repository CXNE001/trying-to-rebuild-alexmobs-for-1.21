...
extends JpegDecoder {
    ...
    void decodeBlock(BitReader bits, int[] dcPredictor, int[][] quant, VLC[] huff, Picture result, int[] buf, int blkX, int blkY, int plane, int chroma, int field, int step) {
        ...
        buf[2] = 0;
        buf[1] = 0;
        dcPredictor[plane] = buf[0] = JpegToThumb4x4.readDCValue(bits, huff[chroma]) * quant[chroma][0] + dcPredictor[plane];
        this.readACValues(bits, buf, huff[chroma + 2], quant[chroma]);
        IDCT4x4.idct(buf, 0);
        JpegToThumb4x4.putBlock4x4(result.getPlaneData(plane), result.getPlaneWidth(plane), buf, blkX, blkY, field, step);
    }

    private static void putBlock4x4(byte[] plane, int stride, int[] patch, int x, int y, int field, int step) {
        int dstride = step * (stride >>= 1);
    ...
    void readACValues(BitReader _in, int[] target, VLC table, int[] quantTable) {
        ...
        do {
            ...
            rle = code >> 4;
            len = code & 0xF;
            target[JpegToThumb4x4.mapping4x4[curOff += rle]] = JpegToThumb4x4.toValue(_in.readNBit(len), len) * quantTable[curOff];
            ++curOff;

...
import com.github.alexthe666.citadel.repack.jcodec.containers.mxf.MXFConst;
...
public class MXFDemuxer {
    ...
    public MXFDemuxerTrack[] findTracks() throws IOException {
        ...
        LinkedHashMap<Integer, MXFDemuxerTrack> tracks = new LinkedHashMap<Integer, MXFDemuxerTrack>();
        for (TimelineTrack track : _tracks) {
            ...
            MXFDemuxerTrack dt = this.createTrack(ul, track, descriptor);
    ...
    public List<SourceClip> getSourceClips(int trackId) {
        ...
        ArrayList<SourceClip> clips = new ArrayList<SourceClip>();
        for (MXFMetadata m : this.metadata) {
            SourceClip clip;
            if (m instanceof TimelineTrack) {
                TimelineTrack tt = (TimelineTrack)m;
                int trackId2 = tt.getTrackId();
                boolean bl = trackFound = trackId2 == trackId;
            }
            if (!trackFound || !(m instanceof SourceClip) || (clip = (SourceClip)m).getSourceTrackId() != trackId) continue;
            clips.add(clip);
        }
        return clips;
    }
    ...
    public static TapeTimecode readTapeTimecode(File mxf) throws IOException {
        ...
        try {
            ...
            if (editRate != null) {
                object = TapeTimecode.tapeTimecode(tc, dropFrame, (int)Math.ceil(editRate.toDouble()));
                return object;
            }
            object = null;
            return object;

...
public class CoeffTransformer {
    ...
    public static void fdct4x4(int[] block) {
        ...
        for (i = 0; i < 4; ++i) {
            t0 = block[i] + block[i + 12];
            t1 = block[i + 4] + block[i + 8];
            t2 = block[i + 4] - block[i + 8];
            t3 = block[i] - block[i + 12];
            block[i] = t0 + t1;
            block[i + 4] = t2 + (t3 << 1);
            block[i + 8] = t0 - t1;
            block[i + 12] = t3 - (t2 << 1);
    ...
    public static void fvdDC4x4(int[] scaled) {
        ...
        int i;
        for (i = 0; i < 16; i += 4) {
            t0 = scaled[i] + scaled[i + 3];
            t1 = scaled[i + 1] + scaled[i + 2];
            t2 = scaled[i + 1] - scaled[i + 2];
            t3 = scaled[i] - scaled[i + 3];
            scaled[i] = t0 + t1;
            scaled[i + 1] = t3 + t2;
            scaled[i + 2] = t0 - t1;
            scaled[i + 3] = t3 - t2;
        ...
        for (i = 0; i < 4; ++i) {
            t0 = scaled[i] + scaled[i + 12];
            t1 = scaled[i + 4] + scaled[i + 8];
            t2 = scaled[i + 4] - scaled[i + 8];
            t3 = scaled[i] - scaled[i + 12];
            scaled[i] = t0 + t1 >> 1;
            scaled[i + 4] = t2 + t3 >> 1;
            scaled[i + 8] = t0 - t1 >> 1;
            scaled[i + 12] = t3 - t2 >> 1;
    ...
    public static void idct8x8(int[] ac) {
        ...
        for (int row = 0; row < 8; ++row) {
            e0 = ac[off] + ac[off + 4];
            e1 = -ac[off + 3] + ac[off + 5] - ac[off + 7] - (ac[off + 7] >> 1);
            e2 = ac[off] - ac[off + 4];
            e3 = ac[off + 1] + ac[off + 7] - ac[off + 3] - (ac[off + 3] >> 1);
            e4 = (ac[off + 2] >> 1) - ac[off + 6];
            e5 = -ac[off + 1] + ac[off + 7] + ac[off + 5] + (ac[off + 5] >> 1);
            e6 = ac[off + 2] + (ac[off + 6] >> 1);
            e7 = ac[off + 3] + ac[off + 5] + ac[off + 1] + (ac[off + 1] >> 1);
            f0 = e0 + e6;
            f1 = e1 + (e7 >> 2);
            f2 = e2 + e4;
            f3 = e3 + (e5 >> 2);
        ...
        for (int col = 0; col < 8; ++col) {
            e0 = ac[col] + ac[col + 32];
            e1 = -ac[col + 24] + ac[col + 40] - ac[col + 56] - (ac[col + 56] >> 1);
            e2 = ac[col] - ac[col + 32];
            e3 = ac[col + 8] + ac[col + 56] - ac[col + 24] - (ac[col + 24] >> 1);
            e4 = (ac[col + 16] >> 1) - ac[col + 48];
            e5 = -ac[col + 8] + ac[col + 56] + ac[col + 40] + (ac[col + 40] >> 1);
            e6 = ac[col + 16] + (ac[col + 48] >> 1);
            e7 = ac[col + 24] + ac[col + 40] + ac[col + 8] + (ac[col + 8] >> 1);
            f0 = e0 + e6;
            f1 = e1 + (e7 >> 2);
            f2 = e2 + e4;
            f3 = e3 + (e5 >> 2);
    ...
    static {
        int i;
        zigzag4x4 = new int[]{0, 1, 4, 8, 5, 2, 3, 6, 9, 12, 13, 10, 7, 11, 14, 15};
        invZigzag4x4 = new int[16];
        dequantCoef = new int[][]{{10, 13, 10, 13, 13, 16, 13, 16, 10, 13, 10, 13, 13, 16, 13, 16}, {11, 14, 11, 14, 14, 18, 14, 18, 11, 14, 11, 14, 14, 18, 14, 18}, {13, 16, 13, 16, 16, 20, 16, 20, 13, 16, 13, 16, 16, 20, 16, 20}, {14, 18, 14, 18, 18, 23, 18, 23, 14, 18, 14, 18, 18, 23, 18, 23}, {16, 20, 16, 20, 20, 25, 20, 25, 16, 20, 16, 20, 20, 25, 20, 25}, {18, 23, 18, 23, 23, 29, 23, 29, 18, 23, 18, 23, 23, 29, 23, 29}};
        dequantCoef8x8 = new int[6][64];
        initDequantCoeff8x8 = new int[][]{{20, 18, 32, 19, 25, 24}, {22, 19, 35, 21, 28, 26}, {26, 23, 42, 24, 33, 31}, {28, 25, 45, 26, 35, 33}, {32, 28, 51, 30, 40, 38}, {36, 32, 58, 34, 46, 43}};
        ...
        quantCoeff = new int[][]{{13107, 8066, 13107, 8066, 8066, 5243, 8066, 5243, 13107, 8066, 13107, 8066, 8066, 5243, 8066, 5243}, {11916, 7490, 11916, 7490, 7490, 4660, 7490, 4660, 11916, 7490, 11916, 7490, 7490, 4660, 7490, 4660}, {10082, 6554, 10082, 6554, 6554, 4194, 6554, 4194, 10082, 6554, 10082, 6554, 6554, 4194, 6554, 4194}, {9362, 5825, 9362, 5825, 5825, 3647, 5825, 3647, 9362, 5825, 9362, 5825, 5825, 3647, 5825, 3647}, {8192, 5243, 8192, 5243, 5243, 3355, 5243, 3355, 8192, 5243, 8192, 5243, 5243, 3355, 5243, 3355}, {7282, 4559, 7282, 4559, 4559, 2893, 4559, 2893, 7282, 4559, 7282, 4559, 4559, 2893, 4559, 2893}};
        for (int g = 0; g < 6; ++g) {
            int j;
            int i2;
            ...
            for (i2 = 0; i2 < 8; i2 += 4) {
                for (j = 0; j < 8; j += 4) {
                    CoeffTransformer.dequantCoef8x8[g][(i2 << 3) + j] = initDequantCoeff8x8[g][0];
                }
            }
            for (i2 = 1; i2 < 8; i2 += 2) {
                for (j = 1; j < 8; j += 2) {
                    CoeffTransformer.dequantCoef8x8[g][(i2 << 3) + j] = initDequantCoeff8x8[g][1];
                }
            }
            for (i2 = 2; i2 < 8; i2 += 4) {
                for (j = 2; j < 8; j += 4) {
                    CoeffTransformer.dequantCoef8x8[g][(i2 << 3) + j] = initDequantCoeff8x8[g][2];
                }
            }
            for (i2 = 0; i2 < 8; i2 += 4) {
                for (j = 1; j < 8; j += 2) {
                    CoeffTransformer.dequantCoef8x8[g][(i2 << 3) + j] = initDequantCoeff8x8[g][3];
                }
            }
            for (i2 = 1; i2 < 8; i2 += 2) {
                for (j = 0; j < 8; j += 4) {
            ...
            for (i2 = 0; i2 < 8; i2 += 4) {
                for (j = 2; j < 8; j += 4) {
                    CoeffTransformer.dequantCoef8x8[g][(i2 << 3) + j] = initDequantCoeff8x8[g][4];
                }
            }
            for (i2 = 2; i2 < 8; i2 += 4) {
                for (j = 0; j < 8; j += 4) {

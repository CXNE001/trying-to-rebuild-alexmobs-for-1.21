...
import com.github.alexthe666.citadel.repack.jcodec.codecs.h264.encode.MBDeblocker;
...
implements SaveRestore {
    ...
    public static void chromaResidual(Picture pic, int mbX, int mbY, BitWriter out, int qp, int[][] ac1, int[][] ac2, CAVLC cavlc1, CAVLC cavlc2, MBType leftMBType, MBType topMBType) {
        ...
        int[] dc1 = MBEncoderI16x16.extractDC(ac1);
        int[] dc2 = MBEncoderI16x16.extractDC(ac2);
        ...
        MBEncoderI16x16.writeAC(cavlc1, mbX, mbY, out, mbX << 1, mbY << 1, ac1, qp, leftMBType, topMBType, DUMMY);
        MBEncoderI16x16.writeAC(cavlc2, mbX, mbY, out, mbX << 1, mbY << 1, ac2, qp, leftMBType, topMBType, DUMMY);
    ...
    private void luma(Picture pic, int mbX, int mbY, BitWriter out, int qp, Picture outMB, CAVLC cavlc) {
        ...
        int[] dc = MBEncoderI16x16.extractDC(ac);
        MBEncoderI16x16.writeDC(cavlc, mbX, mbY, out, qp, mbX << 2, mbY << 2, dc, MBType.I_16x16, MBType.I_16x16);
        MBEncoderI16x16.writeAC(cavlc, mbX, mbY, out, mbX << 2, mbY << 2, ac, qp, MBType.I_16x16, MBType.I_16x16, DUMMY);
    ...
    private void chromaPredBlk0(int comp, int x, int y, byte[] pred) {
        int predY = y & 7;
        int dc = x != 0 && y != 0 ? this.chromaPredTwo(this.leftRow[comp], this.topLine[comp], predY, x) : (x != 0 ? this.chromaPredOne(this.leftRow[comp], predY) : (y != 0 ? this.chromaPredOne(this.topLine[comp], x) : 0));
        int i = 0;
        while (i < pred.length) {
            int n = i++;
            pred[n] = (byte)(pred[n] + dc);
        }
    }

    private void chromaPredBlk1(int comp, int x, int y, byte[] pred) {
        int predY = y & 7;
        int dc = y != 0 ? this.chromaPredOne(this.topLine[comp], x + 4) : (x != 0 ? this.chromaPredOne(this.leftRow[comp], predY) : 0);
        int i = 0;
        while (i < pred.length) {
            int n = i++;
            pred[n] = (byte)(pred[n] + dc);
    ...
    private void chromaPredBlk2(int comp, int x, int y, byte[] pred) {
        int predY = y & 7;
        int dc = x != 0 ? this.chromaPredOne(this.leftRow[comp], predY + 4) : (y != 0 ? this.chromaPredOne(this.topLine[comp], x) : 0);
        int i = 0;
        while (i < pred.length) {
            int n = i++;
            pred[n] = (byte)(pred[n] + dc);
    ...
    private void chromaPredBlk3(int comp, int x, int y, byte[] pred) {
        int predY = y & 7;
        int dc = x != 0 && y != 0 ? this.chromaPredTwo(this.leftRow[comp], this.topLine[comp], predY + 4, x + 4) : (x != 0 ? this.chromaPredOne(this.leftRow[comp], predY + 4) : (y != 0 ? this.chromaPredOne(this.topLine[comp], x + 4) : 0));
        int i = 0;
        while (i < pred.length) {
            int n = i++;
            pred[n] = (byte)(pred[n] + dc);
    ...
    private void lumaDCPred(int x, int y, byte[][] pred) {
        int dc = x == 0 && y == 0 ? 0 : (y == 0 ? ArrayUtil.sumByte(this.leftRow[0]) + 8 >> 4 : (x == 0 ? ArrayUtil.sumByte3(this.topLine[0], x, 16) + 8 >> 4 : ArrayUtil.sumByte(this.leftRow[0]) + ArrayUtil.sumByte3(this.topLine[0], x, 16) + 16 >> 5));
        for (int i = 0; i < pred.length; ++i) {
            int j = 0;
            while (j < pred[i].length) {
                byte[] byArray = pred[i];
                int n = j++;
                byArray[n] = (byte)(byArray[n] + dc);
            }
        }
    }

...
public class VLC {
    ...
    private int invert(int startOff, int level, int prefix, IntArrayList values, IntArrayList valueSizes) {
        ...
        for (int i = 0; i < this.codeSizes.length; ++i) {
            if (this.codeSizes[i] <= prefLen || level > 0 && this.codes[i] >>> 32 - prefLen != prefix) continue;
    ...
    public int readVLC(BitReader _in) {
        ...
        while (len == 0) {
            ...
            _in.skip(bits);
            if (code == -1) {
                throw new RuntimeException("Invalid code prefix " + VLC.binary(overall, (i << 3) + bits));
            }
            ++i;
        }
        return code;
    ...
    private static String binary(int string, int len) {
        ...
        for (int i = 0; i < len; ++i) {
            symb[i] = (string & 1 << len - i - 1) != 0 ? 49 : 48;
        }
    ...
    public void printTable(PrintStream ps) {
        for (int i = 0; i < this.values.length; ++i) {
            ps.println(i + ": " + VLC.extracted(i) + " (" + this.valueSizes[i] + ") -> " + this.values[i]);
        }
    }

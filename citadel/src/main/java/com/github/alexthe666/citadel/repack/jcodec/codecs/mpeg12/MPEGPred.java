...
public class MPEGPred {
    ...
    public void predictPlane(byte[] ref, int refX, int refY, int refW, int refH, int refVertStep, int refVertOff, int[] tgt, int tgtY, int tgtW, int tgtH, int tgtVertStep) {
        ...
        int rx = refX >> 1;
        int ry = refY >> 1;
        boolean bl = safe = rx >= 0 && ry >= 0 && rx + tgtW < refW && ry + tgtH << refVertStep < refH;
        if ((refX & 1) == 0) {
            if ((refY & 1) == 0) {
                if (safe) {
    ...
    private void predict16x16DualPrimeField(Picture[] reference, int x, int y, BitReader bits, int[][] mbPix, int fieldNo) {
        int vect1X = this.mvectDecode(bits, this.fCode[0][0], this.mvPred[0][0][0]);
        int dmX = MPEGConst.vlcDualPrime.readVLC(bits) - 1;
        int vect1Y = this.mvectDecode(bits, this.fCode[0][1], this.mvPred[0][0][1]);
        int dmY = MPEGConst.vlcDualPrime.readVLC(bits) - 1;
        int vect2X = this.dpXField(vect1X, dmX, 1 - fieldNo);
        int vect2Y = this.dpYField(vect1Y, dmY, 1 - fieldNo);
        int ch = this.chromaFormat == 1 ? 1 : 0;
        int cw = this.chromaFormat == 3 ? 0 : 1;
        int sh = this.chromaFormat == 1 ? 2 : 1;
        int sw = this.chromaFormat == 3 ? 1 : 2;
        int[][] mbPix1 = new int[3][256];
        int[][] mbPix2 = new int[3][256];
        ...
        this.mvPred[1][0][0] = n;
        int n2 = vect1Y;
        this.mvPred[0][0][1] = n2;
        this.mvPred[1][0][1] = n2;
    }
    ...
    private void predict16x16DualPrimeFrame(Picture[] reference, int x, int y, BitReader bits, int backward, int[][] mbPix) {
        int vect1X = this.mvectDecode(bits, this.fCode[0][0], this.mvPred[0][0][0]);
        int dmX = MPEGConst.vlcDualPrime.readVLC(bits) - 1;
        int vect1Y = this.mvectDecode(bits, this.fCode[0][1], this.mvPred[0][0][1] >> 1);
        int dmY = MPEGConst.vlcDualPrime.readVLC(bits) - 1;
    ...
    private final int mvectDecode(BitReader bits, int fcode, int pred) {
        ...
        return this.sign_extend(val += pred, 5 + shift);
    }
    ...
    protected void predictGeneric(Picture reference, int x, int y, BitReader bits, int backward, int[][] mbPix, int tgtY, int blkW, int blkH, int isSrcField, int srcField, int isDstField, int vectIdx, int predScale) {
        int vectX = this.mvectDecode(bits, this.fCode[backward][0], this.mvPred[vectIdx][backward][0]);
        int vectY = this.mvectDecode(bits, this.fCode[backward][1], this.mvPred[vectIdx][backward][1] >> predScale);
    ...
    private void predictFieldInFrame(Picture reference, int x, int y, int[][] mbPix, BitReader bits, int backward, int spatial_temporal_weight_code) {
        int field = bits.read1Bit();
        this.predictGeneric(reference, x, y >>= 1, bits, backward, mbPix, 0, 16, 8, 1, field, 1, 0, 1);
    ...
    public void predictMB(Picture ref, int refX, int vectX, int refY, int vectY, int blkW, int blkH, int refVertStep, int refVertOff, int[][] tgt, int tgtY, int tgtVertStep) {
        ...
        this.predictPlane(ref.getPlaneData(1), (refX >> cw) + vectX / sw, (refY >> ch) + vectY / sh, ref.getPlaneWidth(1), ref.getPlaneHeight(1), refVertStep, refVertOff, tgt[1], tgtY, blkW >> cw, blkH >> ch, tgtVertStep);
        this.predictPlane(ref.getPlaneData(2), (refX >> cw) + vectX / sw, (refY >> ch) + vectY / sh, ref.getPlaneWidth(2), ref.getPlaneHeight(2), refVertStep, refVertOff, tgt[2], tgtY, blkW >> cw, blkH >> ch, tgtVertStep);
    }

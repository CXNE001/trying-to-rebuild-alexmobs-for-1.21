...
public class RefListManager {
    ...
    public Frame[][] getRefList() {
        ...
        if (refList != null) {
            for (int l = 0; l < 2; ++l) {
                if (refList[l] == null) continue;
                for (int i = 0; i < refList[l].length; ++i) {
                    if (refList[l][i] == null) continue;
    ...
    private Frame[][] buildRefListB() {
        ...
        if (Platform.arrayEqualsObj(l0, l1 = this.buildList(Frame.POCAsc, Frame.POCDesc)) && this.count((Frame[])l1) > 1) {
            Object frame = l1[1];
            l1[1] = l1[0];
            l1[0] = frame;
        }
        Frame[][] result = new Frame[][]{(Frame[])Platform.copyOfObj(l0, this.numRef[0]), (Frame[])Platform.copyOfObj(l1, this.numRef[1])};
    ...
    private void reorder(Picture[] result, int list) {
        ...
        for (int ind = 0; ind < this.sh.refPicReordering[list][0].length; ++ind) {
            ...
            for (i = this.numRef[list] - 1; i > ind; --i) {
                result[i] = result[i - 1];
            }

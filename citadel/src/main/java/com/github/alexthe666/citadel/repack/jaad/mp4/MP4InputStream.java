...
public class MP4InputStream {
    ...
    public void peek(byte[] b, int off, int len) throws IOException {
        int read;
        int i = 0;
        for (read = 0; read < len && read < this.peeked.size(); ++read) {
            b[off + read] = this.peeked.get(read);
        }
        while (read < len) {
            if (this.in != null) {
    ...
    public void read(byte[] b, int off, int len) throws IOException {
        int read;
        int i = 0;
        for (read = 0; read < len && !this.peeked.isEmpty(); ++read) {
            b[off + read] = this.peeked.remove();
        }
        while (read < len) {
            if (this.in != null) {
                i = this.in.read(b, off + read, len - read);

...
public class IntIntMap {
    ...
    private int[] storage = IntIntMap.createArray(128);
    private int _size;
    ...
    public void put(int key, int val) {
        ...
        if (this.storage.length <= key) {
            int[] ns = IntIntMap.createArray(key + 128);
    ...
    public int[] keys() {
        ...
        for (int i = 0; i < this.storage.length; ++i) {
            if (this.storage[i] == Integer.MIN_VALUE) continue;
            result[r++] = i;
        }
        return result;
    }
    ...
    public int[] values() {
        int[] result = IntIntMap.createArray(this._size);
        int r = 0;
        for (int i = 0; i < this.storage.length; ++i) {
            if (this.storage[i] == Integer.MIN_VALUE) continue;

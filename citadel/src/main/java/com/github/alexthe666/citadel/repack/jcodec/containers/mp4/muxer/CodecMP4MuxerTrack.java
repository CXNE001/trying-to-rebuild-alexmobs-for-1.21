...
import com.github.alexthe666.citadel.repack.jcodec.containers.mp4.muxer.MP4MuxerTrack;
...
public class CodecMP4MuxerTrack
extends MP4MuxerTrack {
    ...
    void addVideoSampleEntry(VideoCodecMeta meta) {
        VideoSampleEntry se = VideoSampleEntry.videoSampleEntry(codec2fourcc.get(this.codec), meta.getSize(), "JCodec");
    ...
    public void setCodecPrivateIfNeeded() {
        if (this.codec == Codec.H264) {
            List<ByteBuffer> sps = CodecMP4MuxerTrack.selectUnique(this.spsList);
            List<ByteBuffer> pps = CodecMP4MuxerTrack.selectUnique(this.ppsList);
    ...
    void addAudioSampleEntry(AudioFormat format) {
        AudioSampleEntry ase = AudioSampleEntry.compressedAudioSampleEntry(codec2fourcc.get(this.codec), 1, 16, format.getChannels(), format.getSampleRate(), 0, 0, 0);

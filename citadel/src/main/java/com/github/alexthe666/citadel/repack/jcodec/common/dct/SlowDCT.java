    ...
    public short[] encode(byte[] orig) {
        ...
        for (int u = 0; u < 8; ++u) {
            for (int v = 0; v < 8; ++v) {
                ...
                for (int i = 0; i < 8; ++i) {
                    for (int j = 0; j < 8; ++j) {
                        sum = (float)((double)sum + (double)orig[i * 8 + j] * Math.cos(0.39269908169872414 * ((double)i + 0.5) * (double)u) * Math.cos(0.39269908169872414 * ((double)j + 0.5) * (double)v));
                    }
                }
        ...
        for (int i = 1; i < 8; ++i) {
            result[i] = (byte)((double)result[0] * sqrt2 / 8.0);
            result[i * 8] = (byte)((double)result[0] * sqrt2 / 8.0);
            for (int j = 1; j < 8; ++j) {
                result[i * 8 + j] = (byte)((float)result[0] / 4.0f);
            }
        }
        return result;
    }
